...
```{r}
# NT$ currency to US Dollar: Divide only the taiwanese currency by 30 and keeps the US currency the same
Data_norep <- Data_norep %>%
...
trait_means_SD <- trait_means_SD %>% 
  ...
  dplyr::summarise(mean_trustworthy = mean(mean_trustworthy),
                   ...
                   mean_competent = mean(mean_competent)) # retrieve single mean value from table
...
Data_noNeut <- Data_noNeut %>% 
  mutate(reputation = factor(reputation, levels = c("high", "mod", "low")))
...
computeDiffs <- Data_noNeut %>% 
  ...
  dplyr::summarise(response_i_mean = mean(response_i, na.rm = T),
            competent_i = mean(trait_competent_i, na.rm = T),
  ...
  pivot_wider(names_from = expression, values_from = c(response_i_mean, competent_i, trustworthy_i, financiallyNeedy_i, dominant_i, friendly_i)) %>% 
  dplyr::mutate(responseRatio = response_i_mean_exci/response_i_mean_calm,
         ...
         competentDiff_i = competent_i_exci - competent_i_calm,
...
# step 2: x -> m 
summary(lm(competentDiff_i ~ Culture, data = mediationData))

# step 3: x -> m 
summary(lm(responseDiff_i ~ competentDiff_i + Culture, data = mediationData))

# set up models
model.M <- lm(competentDiff_i ~ Culture, data = mediationData)
model.Y <- lm(responseDiff_i ~ Culture + competentDiff_i, data = mediationData)

results <- mediation::mediate(model.M, model.Y, treat='Culture', mediator='competentDiff_i', boot = TRUE, sims=1000, control.value="Taiwanese", treat.value="European American")
summary(results)

# for reporting mediator --> DV arrow
summary(lm(responseDiff_i ~ competentDiff_i, data = mediationData))
```
...
# mean center traits
mediationData$competentDiff_i.c <- scale(mediationData$competentDiff_i, center = TRUE, scale = FALSE)[,] #Scale returns a matrix so we have to make it a vector by indexing one column
...
trait_competent <- model7(iv = "Culture_num", dv ="responseDiff_i", med = "competentDiff_i.c", mod = "reputation", data = mediationData, samples = 5000)
kable(trait_competent)
