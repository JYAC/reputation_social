...
summary <- Data_subset %>%
  # calculate for each subject
  dplyr::ungroup() %>%
  dplyr::group_by(pID, Culture, reputation) %>%
  dplyr::summarise(n = n(),
                   mean = mean(responseConvert, na.rm = T),
                   mean_i = mean(response_i, na.rm = T),
                   stdv = sd(responseConvert),
...
summary <- Data_subset %>%
  ...
  # calculate for each subject
  dplyr::ungroup() %>%
  dplyr::group_by(pID, Culture, expression) %>%
  dplyr::summarise(n = n(),
                   mean = mean(responseConvert, na.rm = T),
                   mean_i = mean(response_i, na.rm = T),
                   stdv = sd(responseConvert),
...
summary <- Data_subset %>%
  # calculate for each subject
  dplyr::ungroup() %>%
  dplyr::group_by(pID, Culture, expression, reputation) %>%
  dplyr::summarise(n = n(),
                   mean = mean(response, na.rm = T),
                   mean_i = mean(response_i, na.rm = T),
...
ggplot(summary, aes(x = reputation, y = mean_conv, fill = expression)) +
  ...
  scale_fill_manual(values=c("#bdbdbd","#94b3d7", "#d99594")) +
  xlab("Target Reputation") +
  ylab("Mean Transfer (Simple Conversion $)") +
  theme(axis.title.x = element_text(face = "bold", size = 16),
        axis.text.x = element_text(size = 14),
        strip.text.x = element_text(size = 14),
        axis.title.y = element_text(face = "bold", size = 16),
...
ggplot(summary, aes(x = reputation, y = mean_ppp, fill = expression)) +
  ...
  scale_fill_manual(values=c("#bdbdbd","#94b3d7", "#d99594")) +
  xlab("Target Reputation") +
  ylab("Mean Transfer (PPP Converted $)") +
  theme(axis.title.x = element_text(face = "bold", size = 16),
        axis.text.x = element_text(size = 14),
        strip.text.x = element_text(size = 14),
        axis.title.y = element_text(face = "bold", size = 16),
...
summary_race <- Data_subset %>%
  # calculate for each subject
  dplyr::ungroup() %>%
  dplyr::group_by(pID, Culture, expression, reputation, target_race) %>%
  dplyr::summarize(n = n(),
                   mean = mean(response, na.rm = T),
                   mean_i = mean(response_i, na.rm = T),
                   stdv = sd(response),
...
summary_sex <- Data_subset %>%
  # calculate for each subject
  dplyr::ungroup() %>%
  dplyr::group_by(pID, Culture, expression, reputation, sex) %>%
  dplyr::summarize(n = n(),
                   mean = mean(response, na.rm = T),
                   mean_i = mean(response_i, na.rm = T),
                   stdv = sd(response),
...
summary_race_sex <- Data_subset %>%
  # calculate for each subject
  dplyr::ungroup() %>%
  dplyr::group_by(pID, Culture, expression, reputation, target_race, sex) %>%
  dplyr::summarize(n = n(),
                   mean = mean(response, na.rm = T),
                   mean_i = mean(response_i, na.rm = T),
                   stdv = sd(response),
