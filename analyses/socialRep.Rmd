...
source("../../../../R\ functions/summarySEwithin2.R") # updated summarySEwithin function for unnormed means
```
...
```{r}
# read in part 1 data
Part1_norep <- read.csv("../../Data/US/Part1/one_shot_trust_long_norep.csv")
Part1 <- read.csv("../../Data/US/Part1/one_shot_trust_long.csv")
# read in part 2 data
Part2 <- read.csv("../../Data/US/Part2/reputation_study_part2.csv")
...
```{r}
# read in part 1 data
Part1_norep_t <- read.csv("../../Data/Taiwan/Part1/one_shot_trust_long_norep.csv")
Part1_t <- read.csv("../../Data/Taiwan/Part1/one_shot_trust_long.csv")

# read in part 2 data
# Part2_t <- fetch_survey(surveyID = "SV_5tO8DW4nWx5bGPb", force_request = TRUE, convert = FALSE, label = FALSE)
# write.csv(Part2_t, "../../Data/Taiwan/Part2/reputation_study_part2_t.csv")
Part2_t <- read.csv("../../Data/Taiwan/Part2/reputation_study_part2_t.csv")
...
Data_norep <- Data_norep %>%
  ...
  mutate(Culture = factor(Culture,
                          levels = c("European American", "Taiwanese"),
                          labels = c("European American", "Taiwanese")),
...
Data_norep <- Data_norep %>% 
  dplyr::ungroup() %>% 
  dplyr::group_by(pID) %>%
  # calculate SD in amount transferred
...
### Ipsatize trait ratings (by individual)

```{r}
ipsatizedTraitRatings <- Data %>% 
  dplyr::select(pID, trial_num, trait_competent, trait_trustworthy, trait_financiallyNeedy, trait_dominant, trait_friendly) %>% #Only select variables
  ungroup() %>%
  group_by(pID) %>%
  dplyr::mutate(trait_competent_mean = mean(trait_competent, na.rm = T),
                trait_trustworthy_mean = mean(trait_trustworthy, na.rm = T),
                trait_financiallyNeedy_mean = mean(trait_financiallyNeedy, na.rm = T),
                trait_dominant_mean = mean(trait_dominant, na.rm = T),
                trait_friendly_mean = mean(trait_friendly, na.rm = T),
                
                trait_competent_SD = sd(trait_competent, na.rm = T),
                trait_trustworthy_SD = sd(trait_trustworthy, na.rm = T),
                trait_financiallyNeedy_SD = sd(trait_financiallyNeedy, na.rm = T),
                trait_dominant_SD = sd(trait_dominant, na.rm = T),
                trait_friendly_SD = sd(trait_friendly, na.rm = T),
                
                trait_competent_i = (trait_competent - trait_competent_mean)/trait_competent_SD,
                trait_trustworthy_i = (trait_trustworthy - trait_trustworthy_mean)/trait_trustworthy_SD,
                trait_financiallyNeedy_i = (trait_financiallyNeedy - trait_financiallyNeedy_mean)/trait_financiallyNeedy_SD,
                trait_dominant_i = (trait_dominant - trait_dominant_mean)/trait_dominant_SD,
                trait_friendly_i = (trait_friendly - trait_friendly_mean)/trait_friendly_SD) %>%
  dplyr::select(pID, trial_num, trait_competent_i, trait_trustworthy_i, trait_financiallyNeedy_i, trait_dominant_i, trait_friendly_i) #Keep only pID and ipsatized response scores

Data <- left_join(Data, ipsatizedTraitRatings, by = c("pID", "trial_num")) #Add ipsatized response scores to original dataframe
```
...
# calculate gender for full sample (before exclusions)
Data %>% 
  dplyr::select(Culture, pID, Gender) %>% 
  distinct() %>% 
  dplyr::group_by(Culture, pID, Gender) %>% 
  dplyr::group_by(Culture, Gender) %>% 
...
Total Taiwan sample size (*before* excluding no variation) = `r n_distinct(Data$pID[Data$Culture == "Taiwanese"])`. 
Total Taiwan sample size (*after* excluding no variation) = `r n_distinct(Data$pID[Data$Culture == "Taiwanese"]) - n_distinct(Data$pID[Data$Culture == "Taiwanese" & Data$responseNoVar == 1])`.
...
Data_norep <- Data_norep %>%
  mutate(responseConvert_norep = ifelse(Culture == "Taiwanese", response_norep / 30, response_norep))

Data <- Data %>%
  mutate(responseConvert = ifelse(Culture == "Taiwanese", response / 30, response))
```
...
Data_norep <- Data_norep %>%
  mutate(responsePPP_norep = ifelse(Culture == "Taiwanese", response_norep / 14.06, response_norep))

Data <- Data %>%
  mutate(responsePPP = ifelse(Culture == "Taiwanese", response / 14.06, response))
...
#                     culture = c("Taiwanese","European American"))
...
# get sds
sd(as.numeric(Demographics$FamSES2[Demographics$Culture == "European American"]), na.rm=T) # European Americans 
sd(as.numeric(Demographics$FamSES2[Demographics$Culture == "Taiwanese"]), na.rm=T) # Taiwanese
...
AVI <- Data %>% 
  dplyr::select(pID, Culture, Age, FamSES2, Gender, iHAP, iLAP, iHAP_i, iLAP_i, aHAP, aLAP, aHAP_i, aLAP_i) %>% 
  distinct

contrasts(AVI$Culture) <- cbind(AsianVsEur = c(-1, 1))

# convert SES from factor into numeric
AVI$FamSES2num <- as.numeric(AVI$FamSES2)

# ideal hap, raw
iHAP <- lm(data = AVI, iHAP ~ Culture + aHAP + FamSES2num)
summary(iHAP)

# ideal lap, raw
iLAP <- lm(data = AVI, iLAP ~ Culture + aLAP + FamSES2num)
summary(iLAP)

# ideal hap, ipsatized
iHAP_i <- lm(data = AVI, iHAP_i ~ Culture + aHAP_i + FamSES2num)
summary(iHAP_i)

# ideal lap, ipsatized
iLAP_i <- lm(data = AVI, iLAP_i ~ Culture + aLAP_i + FamSES2num)
summary(iLAP_i)
...
# ipsatized separately for LAP vs. HAP
aov_ips <- aov(iHAP_i ~ Culture + aHAP_i + Error(pID), data = AVI)
summary(aov_ips) # get main effects
eta_sq(aov_ips, partial = TRUE) # get eta squared for partial effects

emm <- emmeans(aov_ips, ~ Culture) # probe interaction
pairs <- pairs(emm) # this outputs p value
confint(pairs) # this outputs confidence interval

# ipsatized separately for LAP vs. HAP
aov_ips <- aov(iLAP_i ~ Culture + aLAP_i + Error(pID), data = AVI)
summary(aov_ips) # get main effects
eta_sq(aov_ips, partial = TRUE) # get eta squared for partial effects
...
# # Archive: running RM ANOVA for iLAP and iHAP together (using AVI_long)
# 
# # ipsatized
...
# only within Taiwaneses (Asi == 0, reference group)
contrasts(AVI_long$Affect) <- cbind(HAPVsLAP = c(1,-1)) # reset
...
Based on [this](https://www.niklasjohannes.com/post/calculating-and-visualizing-error-bars-for-within-subjects-designs/) post, decided to use summarySEwithin function (in Rmisc package) to compute SE's for reporting and plotting.

This method is based on [Morey, 2008](http://pcl.missouri.edu/sites/default/files/morey.2008.pdf). 

However, ended up using the further edited function, [summarySEwithin2](https://gist.github.com/hauselin/a83b6d2f05b0c90c0428017455f73744) because otherwise the "normed" means reported using the summarySEwithin funciton are confusing. 

## Transfers by Culture
...
# compute mean transfer for each participant including neutral
...
# histogram of ipsatized transfers
ggplot(Data_pID, aes(x = give_i)) +
  facet_grid(Culture~.) +
  geom_histogram() +
  xlab("Amount Transferred (ipsatized)") + 
  ylab("Frequency") +
  ggthemes::theme_few() +
  ggtitle("Histogram of Ipsatized Transfers") +
...
# histogram of simply converted transfers
ggplot(Data_pID, aes(x = give_C)) +
  facet_grid(Culture~.) +
  geom_histogram() +
  xlab("Amount Transferred in USD") + 
  ylab("Frequency") +
  ggthemes::theme_few() +
  ggtitle("Histogram of Transfers in USD") +
  theme(axis.title.x = element_text(face = "bold", size = 16),
        axis.text.x = element_text(size = 14),
        strip.text.x = element_text(size = 14),
        axis.title.y = element_text(face = "bold", size = 16),
...
# histogram of PPP converted transfers
ggplot(Data_pID, aes(x = give_PPP)) +
  facet_grid(Culture~.) +
  geom_histogram() +
  xlab("Amount Transferred in USD (PPP Conversion)") + 
  ylab("Frequency") +
  ggthemes::theme_few() +
  ggtitle("Histogram of Transfers in USD (PPP Conversion)") +
  theme(axis.title.x = element_text(face = "bold", size = 16),
        axis.text.x = element_text(size = 14),
        strip.text.x = element_text(size = 14),
        axis.title.y = element_text(face = "bold", size = 16),
...
## Tranfers by Expression

```{r}
# ipsatized transfers

ips_summary <- summarySEwithin2(Data_subset, 
                                measurevar="response_i_norep", 
                                withinvars=c("expression_norep"),
                                idvar="pID", 
                                na.rm=T)

ips_summary %>% pander

ggplot(ips_summary, aes(x=expression_norep, y=response_i_norep, fill=expression_norep)) +
  geom_bar(position=position_dodge(.55), stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=response_i_norep-se, ymax=response_i_norep+se)) +
  scale_x_discrete(name = "Target Expression", labels = c("Neutral", "Calm", "Excited")) +
  scale_fill_manual(values=c("#bdbdbd","#94b3d7", "#d99594")) +
  xlab("Target Expression") +
  ylab("Ipsatized Transfer") +
  theme(axis.title.x = element_text(face = "bold", size = 16),
        axis.text.x = element_text(size = 14),
        strip.text.x = element_text(size = 14),
        axis.title.y = element_text(face = "bold", size = 16),
  ...
  coord_cartesian(y=c(-0.35,0.3))

# raw transfers

raw_summary <- summarySEwithin2(Data_subset, 
                                measurevar="responseConvert_norep", 
                                withinvars=c("expression_norep"), 
                                idvar="pID", 
                                na.rm=T)
...
ggplot(raw_summary, aes(x=expression_norep, y=responseConvert_norep, fill=expression_norep)) +
  geom_bar(position=position_dodge(.55), stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=responseConvert_norep-se, ymax=responseConvert_norep+se)) +
  scale_x_discrete(name = "Target Expression", labels = c("Neutral", "Calm", "Excited")) +
  scale_fill_manual(values=c("#bdbdbd","#94b3d7", "#d99594")) +
  xlab("Target Expression") +
  ylab("Ipsatized Transfer ($)") +
...
## Transfers by Culture and Expression
...
ips_summary <- summarySEwithin2(Data_subset, 
                                measurevar="response_i_norep", 
                                withinvars=c("expression_norep"), 
                                betweenvars=c("Culture"), 
                                idvar="pID", 
                                na.rm=T)

ips_summary %>% pander

ggplot(ips_summary, aes(x=Culture, y=response_i_norep, fill=expression_norep)) +
  geom_bar(position=position_dodge(.55), stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=response_i_norep-se, ymax=response_i_norep+se)) +
...
# for paper, culture as color and expression on x axis

ggplot(ips_summary, aes(x=expression_norep, y=response_i_norep, fill=Culture)) +
  geom_bar(position=position_dodge(.55), stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=response_i_norep-se, ymax=response_i_norep+se)) +
  scale_fill_manual(name = "Culture", labels = c("European American", "Taiwanese"), values=c("#d99594", "#94b3d7")) + # update legend title, labels, and color at once
  scale_x_discrete(name = "Target Expression", labels = c("Neutral", "Calm", "Excited")) +
  ylab("Ipsatized Transfer") +
  coord_cartesian(y=c(-0.5,0.5)) +
  theme_apa()
...
# no neutral
ips_summary_noNeut <- ips_summary %>%
  dplyr::filter(expression_norep != "neut") 

ggplot(ips_summary_noNeut, aes(x=expression_norep, y=response_i_norep, fill=Culture)) +
  geom_bar(position=position_dodge(.55), stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=response_i_norep-se, ymax=response_i_norep+se)) +
  scale_fill_manual(name = "Culture", labels = c("European American", "Taiwanese"), values=c("#d99594", "#94b3d7")) + # update legend title, labels, and color at once
  scale_x_discrete(name = "Target Expression", labels = c("Calm", "Excited")) +
  ylab("Ipsatized Transfer") +
  coord_cartesian(y=c(-0.5,0.5)) +
  theme_apa()
...
ggplot(raw_summary, aes(x=Culture, y=responseConvert_norep, fill=expression_norep)) +
  geom_bar(position=position_dodge(.55), stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=responseConvert_norep-se, ymax=responseConvert_norep+se)) +
  scale_fill_manual(name = "Target Expression", labels = c("Neutral", "Calm", "Excited"), values=c("#bdbdbd","#94b3d7", "#d99594")) + # update legend title, labels, and color at once
  xlab("Culture") +
...
# data with no neutral expression
Data_subset_noNeut <- Data_subset %>%
...
## Transfers by Reputation only

```{r, echo=FALSE}
# ipsatized transfers

ips_summary <- summarySEwithin2(Data_subset, 
                                measurevar="response_i", 
                                withinvars=c("reputation"),
                                idvar="pID", 
                                na.rm=T)

ips_summary %>% pander

ggplot(ips_summary, aes(x=reputation, y=response_i)) +
  geom_bar(position=position_dodge(.55), fill="#D3D3D3", stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=response_i-se, ymax=response_i+se)) +
  scale_x_discrete(name = "Target Reputation", labels = c("Low", "Moderate", "High")) +
...
ggplot(raw_summary, aes(x=reputation, y=responseConvert)) +
  geom_bar(position=position_dodge(.55), fill="#D3D3D3", stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=responseConvert-se, ymax=responseConvert+se)) +
  scale_x_discrete(name = "Target Reputation", labels = c("Low", "Moderate", "High")) +
  xlab("Target Reputation") +
  ylab("Raw Transfer ($)") +
  theme(axis.title.x = element_text(face = "bold", size = 16),
...
ips_summary <- summarySEwithin2(Data_subset, 
                                measurevar="response_i", 
                                withinvars=c("expression"),
                                idvar="pID", 
                                na.rm=T)

ips_summary %>% pander

ggplot(ips_summary, aes(x=expression, y=response_i, fill=expression)) +
  geom_bar(position=position_dodge(.55), stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=response_i-se, ymax=response_i+se)) +
  scale_x_discrete(name = "Target Expression", labels = c("Neutral", "Calm", "Excited")) +
  scale_fill_manual(values=c("#bdbdbd","#94b3d7", "#d99594")) +
...
ggplot(raw_summary, aes(x=expression, y=responseConvert, fill=expression)) +
  geom_bar(position=position_dodge(.55), stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=responseConvert-se, ymax=responseConvert+se)) +
  scale_x_discrete(name = "Target Expression", labels = c("Neutral", "Calm", "Excited")) +
  scale_fill_manual(values=c("#bdbdbd","#94b3d7", "#d99594")) +
...
## Transfers by Culture and Reputation
...
ggplot(ips_summary, aes(x=reputation, y=response_i, fill=Culture)) +
  geom_bar(position=position_dodge(.55), stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=response_i-se, ymax=response_i+se)) +
  scale_fill_manual(values=c("#d99594", "#94b3d7")) +
  xlab("Target Reputation") +
...
ggplot(raw_summary, aes(x=reputation, y=responseConvert, fill=Culture)) +
  geom_bar(position=position_dodge(.55), stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=responseConvert-se, ymax=responseConvert+se)) +
  scale_fill_manual(values=c("#d99594", "#94b3d7")) +
  xlab("Target Reputation") +
...
ggplot(ips_summary, aes(x=Culture, y=response_i, fill=expression)) +
  geom_bar(position=position_dodge(.55), stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=response_i-se, ymax=response_i+se)) +
  scale_fill_manual(name = "Target Expression", labels = c("Neutral", "Calm", "Excited"), values=c("#bdbdbd","#94b3d7", "#d99594")) +
  xlab("Culture") +
  ylab("Ipsatized Transfer") +
...
ggplot(raw_summary, aes(x=Culture, y=responseConvert, fill=expression)) +
  geom_bar(position=position_dodge(.55), stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=responseConvert-se, ymax=responseConvert+se)) +
  scale_fill_manual(name = "Target Expression", labels = c("Neutral", "Calm", "Excited"), values=c("#bdbdbd","#94b3d7", "#d99594")) +
  xlab("Culture") +
...
## Transfers by Expression and Reputation
...
ips_summary <- summarySEwithin2(Data_subset, 
                                measurevar="response_i", 
                                withinvars=c("reputation", "expression"), 
                                idvar="pID", 
                                na.rm=T)

ips_summary %>% pander

ggplot(ips_summary, aes(x=reputation, y=response_i, fill=expression)) +
  geom_bar(position=position_dodge(.55), stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=response_i-se, ymax=response_i+se)) +
  scale_fill_manual(name = "Target Expression", labels = c("Neutral", "Calm", "Excited"), values=c("#bdbdbd","#94b3d7", "#d99594")) +
  xlab("Target Reputation") +
...
ggplot(raw_summary, aes(x=reputation, y=responseConvert, fill=expression)) +
  geom_bar(position=position_dodge(.55), stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=responseConvert-se, ymax=responseConvert+se)) +
  scale_fill_manual(name = "Target Expression", labels = c("Neutral", "Calm", "Excited"), values=c("#bdbdbd","#94b3d7", "#d99594")) +
  xlab("Target Reputation") +
...
## Transfers by Culture, Expression and Reputation
...
ips_summary <- summarySEwithin2(Data_subset, 
                                measurevar="response_i", 
                                withinvars=c("expression", "reputation"),
                                betweenvars=c("Culture"),
                                idvar="pID", 
                                na.rm=T)
...
# change underlying factor level names for plotting:
levels(ips_summary$reputation) <- c("Low", "Mod", "High")
...
ggplot(ips_summary, aes(x=reputation, y=response_i, fill=expression)) +
  ...
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=response_i-se, ymax=response_i+se)) +
  scale_fill_manual(name = "Target Expression", labels = c("Neutral", "Calm", "Excited"), values=c("#bdbdbd","#94b3d7", "#d99594")) +
  scale_x_discrete(labels=c("low" = "Low", "mod" = "Mod", "high" = "High")) +
  xlab("Target Reputation") +
  ylab("Ipsatized Transfer") +
  theme(axis.title.x = element_text(face = "bold", size = 16),
        axis.text.x = element_text(size = 14),
        ...
        legend.title = element_text(face = "bold", size = 16),
        legend.text = element_text(size = 14)) + 
  coord_cartesian(y=c(-1.1, 1.30)) + # make room for asterisks
  theme_apa()

# highlight Taiwan high rep excited: expression on x axis, reputation as facets, culture as color

# change underlying factor level names for plotting:
levels(ips_summary$reputation) <- c("Low Reputation", "Mod Reputation", "High Reputation")

ggplot(ips_summary, aes(x=expression, y=response_i, fill=Culture)) +
  facet_grid(~reputation) +
  geom_bar(position=position_dodge(.55), stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=response_i-se, ymax=response_i+se)) +
  scale_fill_manual(name = "Culture", labels = c("European American", "Taiwanese"), values=c("#d99594", "#94b3d7")) +
...
ips_summary_noNeut <- summarySEwithin2(Data_subset_noNeut, 
                               measurevar="response_i", 
                               withinvars=c("expression", "reputation"),
                               betweenvars=c("Culture"),
                               idvar="pID", 
...
ggplot(ips_summary_noNeut, aes(x=reputation, y=response_i, fill=expression)) +
  facet_grid(~Culture) +
  geom_bar(position=position_dodge(.55), stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=response_i-se, ymax=response_i+se)) +
  scale_fill_manual(name = "Target Expression", labels = c("Calm", "Excited"), values=c("#94b3d7", "#d99594")) +
  scale_x_discrete(labels=c("low" = "Low", "mod" = "Mod", "high" = "High")) +
  xlab("Target Reputation") +
  ylab("Ipsatized Transfer") +
  theme(axis.title.x = element_text(face = "bold", size = 16),
...
# make reputation facet to match no rep plots
# change underlying factor level names for plotting:
levels(ips_summary_noNeut$reputation) <- c("Low Reputation", "Mod Reputation", "High Reputation")

ggplot(ips_summary_noNeut, aes(x=expression, y=response_i, fill=Culture)) +
  facet_grid(~reputation) +
  geom_bar(position=position_dodge(.55), stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=response_i-se, ymax=response_i+se)) +
    scale_x_discrete(name = "Target Expression", labels=c("calm" = "Calm", "exci" = "Excited")) +
  scale_fill_manual(name = "Culture", labels = c("European American", "Taiwanese"), values=c("#d99594", "#94b3d7")) +
  ylab("Ipsatized Transfer") +
  theme(axis.title.x = element_text(face = "bold", size = 16),
        axis.text.x = element_text(size = 14),
...
## Transfers by Culture, Expression, Reputation and *Race*
...
ips_summary <- summarySEwithin2(Data_subset, 
                                measurevar="response_i", 
                                withinvars=c("expression", "reputation", "target_race"),
                                betweenvars=c("Culture"),
                                idvar="pID", 
                                na.rm=T)
...
## Transfers by Culture, Expression, Reputation and *Sex*
...
ips_summary <- summarySEwithin2(Data_subset, 
                                measurevar="response_i", 
                                withinvars=c("expression", "reputation", "sex"),
                                betweenvars=c("Culture"),
                                idvar="pID", 
                                na.rm=T)
...
## Transfers by Culture, Expression, Reputation, Race *and* Sex
...
ips_summary <- summarySEwithin2(Data_subset, 
                                measurevar="response_i", 
                                withinvars=c("expression", "reputation", "target_race", "sex"),
                                betweenvars=c("Culture"),
                                idvar="pID", 
                                na.rm=T)
...
## Transfers by Expression, Reputation, and target Race/target Sex (No Culture)
...
```{r}
# Culture, reputation, expression and participant gender:
...
ips_summary <- summarySEwithin2(Data_subset, 
                                measurevar="response_i", 
                                withinvars=c("expression", "reputation"),
                                betweenvars=c("Culture", "Gender"),
                                idvar="pID", 
                                na.rm=T)

ips_summary %>% pander

ggplot(subset(ips_summary, !is.na(Gender)), aes(x=reputation, y=response_i, fill=expression)) +
  facet_grid(Culture ~ Gender) +
  geom_bar(position=position_dodge(.55), stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=response_i-se, ymax=response_i+se)) +
  scale_fill_manual(name = "Target Expression", labels = c("Neutral", "Calm", "Excited"), values=c("#bdbdbd","#94b3d7", "#d99594")) +
  scale_x_discrete(labels=c("low" = "Low", "mod" = "Mod", "high" = "High")) +
...
ggplot(subset(raw_summary, !is.na(Gender)), aes(x=reputation, y=responseConvert, fill=expression)) +
  facet_grid(Culture ~ Gender) +
  geom_bar(position=position_dodge(.55), stat="identity", width=.5) +
  geom_errorbar(position=position_dodge(.55), width=.25, aes(ymin=responseConvert-se, ymax=responseConvert+se)) +
  scale_fill_manual(name = "Target Expression", labels = c("Neutral", "Calm", "Excited"), values=c("#bdbdbd","#94b3d7", "#d99594")) +
...
# Culture, reputation, expression, participant gender and *target sex*:
...
## Variance explained (inspired by Sam Gershman)
...
model_ipsatized0 <- lmer(data = Eur_Data, response_i ~ 1 + reputation + (1|pID), REML=FALSE) # dv = response_i
summary(model_ipsatized0)

model_ipsatized1 <- lmer(data = Eur_Data, response_i ~ 1 + reputation + expression + (1|pID), REML=FALSE) # dv = response_i
summary(model_ipsatized1)

model_ipsatized2 <- lmer(data = Eur_Data, response_i ~ 1 + reputation * expression + (1|pID), REML=FALSE) # dv = response_i
summary(model_ipsatized2)

anova(model_ipsatized1, model_ipsatized2, refit=FALSE)

# East Asians 
Taiwan_Data <- Data %>% 
  filter(Culture == "Taiwanese")

model_ipsatized0 <- lmer(data = Taiwan_Data, response_i ~ 1 + reputation + (1|pID), REML=FALSE) # dv = response_i
summary(model_ipsatized0)

model_ipsatized1 <- lmer(data = Taiwan_Data, response_i ~ 1 + reputation + expression + (1|pID), REML=FALSE) # dv = response_i
summary(model_ipsatized1)

model_ipsatized2 <- lmer(data = Taiwan_Data, response_i ~ 1 + reputation * expression + (1|pID), REML=FALSE) # dv = response_i
summary(model_ipsatized2)
...
```{r}
aov_ipsatized <- aov(data = Data_norep, response_i_norep ~ Culture * expression_norep + Error(pID/(expression_norep)), na.action = na.exclude) # dv = response_i
summary(aov_ipsatized) # get main effects
eta_sq(aov_ipsatized, partial = TRUE) # get eta squared for partial effects

emm <- emmeans(aov_ipsatized, ~ expression_norep) # probe
pairs <- pairs(emm) # this outputs p value
confint(pairs) # this outputs confidence interval

emm <- emmeans(aov_ipsatized, ~ Culture * expression_norep) # probe
pairs <- pairs(emm) # this outputs p value
confint(pairs) # this outputs confidence interval
```

### No neutral expressions

```{r}
# subset data to just expression levels "calm" vs. "excited" (without "neutral")
Data_norep_noNeut <- Data_norep %>%
  dplyr::filter(expression_norep != "neut") %>%
  # refactor to exclude neutral
  dplyr::mutate(expression_norep = factor(expression_norep, levels = c("calm", "exci")))

# no neut
aov_ipsatized <- aov(data = Data_norep_noNeut, response_i_norep ~ Culture * expression_norep + Error(pID/(expression_norep)), na.action = na.exclude) # dv = response_i
summary(aov_ipsatized) # get main effects
eta_sq(aov_ipsatized, partial = TRUE) # get eta squared for partial effects
...
contrasts(Data_norep$expression_norep)   <- cbind(CalmVsExci = c(0, 1, -1),
                                                  orthogonal = c(2, -1, -1))
# expression: neutral vs. calm
contrasts(Data_norep$expression_norep)   <- cbind(NeutVsCalm = c(1, -1, 0),
                                                  orthogonal = c(-1, -1, 2))
# expression: excited vs. neutral
contrasts(Data_norep$expression_norep)   <- cbind(ExciVsNeut = c(-1, 0, 1),
                                                  orthogonal = c(-1, 2, -1))
...
### probe Culture x Expression (calm/excited) interaction
```{r}
# expression: calm vs. excited
contrasts(Data_norep$expression_norep)   <- cbind(CalmVsExci = c(0, 1, -1),
                                                  orthogonal = c(2, -1, -1))
...
# probe expression for Taiwanese (Asian == 0, reference)
...
contrasts(Data_norep$expression_norep)   <- cbind(exciRef1 = c(1, 0, 0),
                                                  exciRef2 = c(0, 1, 0))
contrasts(Data_norep$Culture) <- cbind(AsianVsEur = c(-1, 1))
...
contrasts(Data_norep$expression_norep)   <- cbind(calmRef1 = c(1, 0, 0),
                                                  calmRef2 = c(0, 0, 1))
contrasts(Data_norep$Culture) <- cbind(AsianVsEur = c(-1, 1))
...
### probe Culture x Expression (excited/neutral) interaction
```{r}
# expression: excited vs. neutral
contrasts(Data_norep$expression_norep)   <- cbind(ExciVsNeut = c(-1, 0, 1),
                                                  orthogonal = c(-1, 2, -1))
...
# only neutral expressions (neutral == 0, reference)
contrasts(Data_norep$expression_norep)   <- cbind(neutRef1 = c(0, 1, 0),
                                                  neutRef2 = c(0, 0, 1))
contrasts(Data_norep$Culture) <- cbind(AsianVsEur = c(-1, 1))
...
# emm <- emmeans(aov_ipsatized, ~ reputation) # probe
# pairs <- pairs(emm) # this outputs p value
# confint(pairs) # this outputs confidence interval
# 
# emm <- emmeans(aov_ipsatized, ~ expression) # probe
# pairs <- pairs(emm) # this outputs p value
# confint(pairs) # this outputs confidence interval
# 
...
# confint(pairs) # this outputs confidence interval
# 
# emm <- emmeans(aov_ipsatized, ~ Culture * expression | reputation) # probe
# pairs <- pairs(emm, simple = list("Culture", c("expression", "reputation")) # this outputs p value
# confint(pairs) # this outputs confidence interval
```
...
# no neut
aov_ipsatized <- aov(data = Data_noNeut, response_i ~ Culture * reputation * expression + Error(pID/(reputation*expression)), na.action = na.exclude) # dv = response_i
summary(aov_ipsatized) # get main effects
eta_sq(aov_ipsatized, partial = TRUE) # get eta squared for partial effects
...
# # controlling for Age and FamSES2num
# aov_ipsatized <- aov(data = Data_noNeut, response_i ~ Culture * reputation * expression + FamSES2num + Error(pID/(reputation*expression)), na.action = na.exclude) # dv = response_i
# summary(aov_ipsatized) # get main effects
# eta_sq(aov_ipsatized, partial = TRUE) # get eta squared for partial effects
# 
# emm <- emmeans(aov_ipsatized, ~ Culture * expression | reputation) # probe
...
aov_ipsatized_target_race <- aov(data = Data_noNeut, response_i ~ Culture * reputation * expression * target_race + Error(pID/(reputation*expression)), na.action = na.exclude) # dv = response_i
summary(aov_ipsatized_target_race) # get main effects
eta_sq(aov_ipsatized_target_race, partial = TRUE) # get eta squared for partial effects

aov_ipsatized_sex <- aov(data = Data_noNeut, response_i ~ Culture * reputation * expression * sex + Error(pID/(reputation*expression)), na.action = na.exclude) # dv = response_i
summary(aov_ipsatized_sex) # get main effects
eta_sq(aov_ipsatized_sex, partial = TRUE) # get eta squared for partial effects
```
...
## Run model comparison for omnibus test of 3 way interaction

```{r}
# ipsatized 

model_ipsatized1 <- lmer(data = Data, response_i ~ 1 + Culture * expression + Culture * reputation + reputation * expression + Gender + Age + FamSES2num + (1|pID), REML=FALSE) # dv = response_i
summary(model_ipsatized1)

model_ipsatized2 <- lmer(data = Data, response_i ~ 1 + reputation * Culture * expression + Gender + Age + FamSES2num + (1|pID), REML=FALSE) # dv = response_i
summary(model_ipsatized2)
...
model_raw1 <- lmer(data = Data, responseConvert ~ 1 + Culture * expression + Culture * reputation + reputation * expression + Gender + Age + FamSES2num + (1|pID), REML=FALSE) # dv = response_i
summary(model_raw1)

model_raw2 <- lmer(data = Data, responseConvert ~ 1 + reputation * Culture * expression + Gender + Age + FamSES2num + (1|pID), REML=FALSE) # dv = response_i
summary(model_raw2)

anova(model_raw1, model_raw2, refit=FALSE)
```
...
### Probe Culture x Reputation x Expression (neut vs. excited) interaction: 

#### Culture --> Reputation --> Expression 
```{r}
# reset
contrasts(Data$expression)   <- cbind(ExciVsNeut = c(-1, 0, 1),
                                      orthogonal = c(-1, 2, -1))

# # only marg sig 3 way for low vs. high rep (linear)
# contrasts(Data$expression)   <- cbind(CalmVsExci = c(0, 1, -1),
#                                       orthogonal = c(2, -1, -1))

# reputation: mod vs. high
contrasts(Data$reputation)   <- cbind(LowVsMod = c(1, -1, 0),
                                      HighVsMod = c(0, -1, 1))

# reputation: low vs. high
contrasts(Data$reputation)   <- cbind(reput_linear = c(-1, 0, 1),
                                      reput_quad = c(-1, 2, -1))

# probe Reputation x Expression interaction for Eur Americans (Eur American == 0, reference)
contrasts(Data$Culture) <- cbind(EurAmerRef = c(0,1))

model_ipsatized <- lmer(data = Data, response_i ~ 1 + Culture * reputation * expression + Gender + Age + FamSES2num + (1|pID), REML=FALSE) # dv = response_i
summary(model_ipsatized)

# probe Reputation x Expression interaction for Taiwanese (Asian == 0, reference)
contrasts(Data$Culture) <- cbind(AsianRef = c(1,0))

model_ipsatized <- lmer(data = Data, response_i ~ 1 + Culture * reputation * expression + Gender + Age + FamSES2num + (1|pID), REML=FALSE) # dv = response_i
summary(model_ipsatized)

# probe Taiwanese Reputation x Expression interaction by isolating by Reputation (Low == 0, reference)
contrasts(Data$reputation)   <- cbind(LowRef1 = c(0, 1, 0),
                                      LowRef2 = c(0, 0, 1))
...
# probe Taiwanese Reputation x Expression interaction by isolating by Reputation (Mod == 0, reference)
contrasts(Data$reputation)   <- cbind(ModRef1 = c(1, 0, 0),
                                      ModRef2 = c(0, 0, 1))
...
# probe Taiwanese Reputation x Expression interaction by isolating by Reputation (High == 0, reference)
contrasts(Data$reputation)   <- cbind(HighRef1 = c(1, 0, 0),
                                      HighRef2 = c(0, 1, 0))
...
# probe Culture x Expression interaction for low Reputation (low == 0, reference)
contrasts(Data$reputation)   <- cbind(lowRef1 = c(0, 1, 0),
                                      lowRef2 = c(0, 0, 1))
...
# probe Culture x Expression interaction for mod Reputation (mod == 0, reference)
contrasts(Data$reputation)   <- cbind(modRef1 = c(1, 0, 0),
                                      modRef2 = c(0, 0, 1))
...
# probe Culture x Expression interaction for high Reputation (high == 0, reference)
contrasts(Data$reputation)   <- cbind(highRef1 = c(1, 0, 0),
                                      highRef2 = c(0, 1, 0))
...
# probe Culture x Expression in high Reputation interaction by isolating by Expression (Excited == 0, reference)
contrasts(Data$reputation)   <- cbind(highRef1 = c(1, 0, 0),
                                      highRef2 = c(0, 1, 0))
contrasts(Data$expression)   <- cbind(exciRef1 = c(1, 0, 0),
                                      exciRef2 = c(0, 1, 0))
...
# probe Culture x Expression in  high Reputation interaction by isolating by Expression (neut == 0, reference)
contrasts(Data$reputation)   <- cbind(highRef1 = c(1, 0, 0),
                                      highRef2 = c(0, 1, 0))
contrasts(Data$expression)   <- cbind(neutRef1 = c(0, 1, 0),
                                      neutRef2 = c(0, 0, 1))
...
# probe Culture x Expression in high Reputation interaction by isolating by Culture (Eur American == 0, reference)
contrasts(Data$reputation)   <- cbind(highRef1 = c(1, 0, 0),
                                      highRef2 = c(0, 1, 0))
contrasts(Data$Culture) <- cbind(eurRef = c(0,1))
...
# probe Culture x Expression in high Reputation interaction by isolating by Culture (Asian == 0, reference)
contrasts(Data$reputation)   <- cbind(highRef1 = c(1, 0, 0),
                                      highRef2 = c(0, 1, 0))
contrasts(Data$Culture) <- cbind(asianRef = c(1,0))
...
#### Expression --> Culture --> Reputation
```{r}
# Start splitting by expression
# reset all coding
contrasts(Data$expression)   <- cbind(ExciVsNeut = c(-1, 0, 1),
                                      orthogonal = c(-1, 2, -1))
...
# probe Culture x Reputation interaction for exci Expression (exci == 0, reference)
contrasts(Data$expression) <- cbind(exciRef1 = c(1, 0, 0),
                                    exciRef2 = c(0, 1, 0))
...
# probe Culture x Reputation interaction for neut Expression (neut == 0, reference)
contrasts(Data$expression) <- cbind(neutRef1 = c(0, 0, 1),
                                    neutRef2 = c(0, 1, 0))

model_ipsatized <- lmer(data = Data, response_i ~ 1 + Culture * reputation * expression + Gender + Age + FamSES2num + (1|pID), REML=FALSE) # dv = response_i
summary(model_ipsatized)

# probe Culture x Reputation in excited Expression interaction by isolating by low reputation (Low == 0, reference)
contrasts(Data$expression) <- cbind(exciRef1 = c(1, 0, 0),
                                    exciRef2 = c(0, 1, 0))
contrasts(Data$reputation)   <- cbind(lowRef1 = c(0, 0, 1),
                                      lowRef2 = c(0, 1, 0))

model_ipsatized <- lmer(data = Data, response_i ~ 1 + Culture * reputation * expression + Gender + Age + FamSES2num + (1|pID), REML=FALSE) # dv = response_i
summary(model_ipsatized)

# probe Culture x Reputation in excited Expression interaction by isolating by mod reputation (Mod == 0, reference)
contrasts(Data$expression) <- cbind(exciRef1 = c(1, 0, 0),
                                    exciRef2 = c(0, 1, 0))
contrasts(Data$reputation)   <- cbind(modRef1 = c(0, 0, 1),
                                      modRef2 = c(1, 0, 0))

model_ipsatized <- lmer(data = Data, response_i ~ 1 + Culture * reputation * expression + Gender + Age + FamSES2num + (1|pID), REML=FALSE) # dv = response_i
summary(model_ipsatized)

# probe Culture x Reputation in excited Expression interaction by isolating by high reputation (High == 0, reference)
contrasts(Data$expression) <- cbind(exciRef1 = c(1, 0, 0),
                                    exciRef2 = c(0, 1, 0))
contrasts(Data$reputation)   <- cbind(highRef1 = c(0, 1, 0),
                                      highRef2 = c(1, 0, 0))
...
### Probe Culture x Reputation x Expression (calm vs. excited) interaction:
Rerun the above code for Culture x Reputation x Expression (neut vs. excited) after running the following code and commenting out any code above that relevels expression the other way: 
```{r}
contrasts(Data$expression)   <- cbind(CalmVsExci = c(0, 1, -1),
                                      orthogonal = c(2, -1, -1))
```
...
# Taiwanese: linear trends for reputation, effect coding expression
...
# Taiwanese: effect coding both expression and reputation
...
```{r}
#Model 1: Player 2 expression, reputation, and ideal HAP as predictors for amount transferred by participants, controlling for actual HAP 
...
#Model 2: Player 2 expression, reputation, and ideal LAP as predictors for amount transferred by participants, controlling for actual LAP 
...
Data %>% 
  ...
  ylab("Amount Transferred (ipsatized)") 
...
# Trait ratings summary statistics 
To figure out how this compares to the "default" without reputation information

```{r}
# compute means and SD for trustworthiness, friendliness, and competence
trait_means_SD_social_rep <- Data_noNeut %>% 
  dplyr::group_by(Culture, expression, reputation) %>% 
  dplyr::summarise(mean_trustworthy = mean(trait_trustworthy, na.rm=T),
                   SD_trustworthy = sd(trait_trustworthy, na.rm=T),
                   
                   mean_friendly = mean(trait_friendly, na.rm=T),
                   SD_friendly = sd(trait_friendly, na.rm=T),
                   
                   mean_competent = mean(trait_competent, na.rm=T),
                   SD_competent = sd(trait_competent, na.rm=T)) %>% 
  mutate_if(is.numeric, round, 2)

trait_means_SD_social_rep

# for this part, first run Study 1 script so you have trait_means_SD available
trait_means_SD <- trait_means_SD %>% 
  group_by(Country) %>% 
  dplyr::summarise(mean_trustworthy = mean(mean_trustworthy),
                   mean_friendly = mean(mean_friendly), 
                   mean_competent = mean(mean_competence)) # retrieve single mean value from table

# Trait plots without no-rep means overlaid on top

# Trustworthy
ggplot(Data_noNeut, aes(x = trait_trustworthy, fill = Culture)) +
  geom_bar() +
  facet_grid(reputation~Culture) +
  xlab("Trustworthy") +
  coord_cartesian(x = c(1,7))

# Friendly
ggplot(Data_noNeut, aes(x = trait_friendly, fill = Culture)) +
  geom_bar() +
  facet_grid(reputation~Culture) +
  xlab("Friendly") +
  coord_cartesian(x = c(1,7))
...
# Competence
ggplot(Data_noNeut, aes(x = trait_competent, fill = Culture)) +
  geom_bar() +
  facet_grid(reputation~Culture) +
  xlab("Competent") +
  coord_cartesian(x = c(1,7))

# Now same plots with Study 1 means overlaid 
...
ggplot(Data_noNeut, aes(x = trait_trustworthy, fill = Culture)) +
  ...
  xlab("Trustworthy") +
  coord_cartesian(x = c(1,7)) +
  geom_vline(data=filter(Data_noNeut, Culture=="European American"), aes(xintercept=trait_means_SD$mean_trustworthy[trait_means_SD$Country == "EA"]), colour = "red", size = 1.5) +
  geom_vline(data=filter(Data_noNeut, Culture=="Taiwanese"), aes(xintercept=trait_means_SD$mean_trustworthy[trait_means_SD$Country == "KR"]), colour = "light grey", size = 1.5) +
  geom_vline(data=filter(Data_noNeut, Culture=="Taiwanese"), aes(xintercept=trait_means_SD$mean_trustworthy[trait_means_SD$Country == "CN"]), colour = "grey", size = 1.5) +
  geom_vline(data=filter(Data_noNeut, Culture=="Taiwanese"), aes(xintercept=trait_means_SD$mean_trustworthy[trait_means_SD$Country == "TW"]), colour = "dark blue", size = 1.5)
...
ggplot(Data_noNeut, aes(x = trait_friendly, fill = Culture)) +
  ...
  xlab("Friendly") +
  coord_cartesian(x = c(1,7)) +
  geom_vline(data=filter(Data_noNeut, Culture=="European American"), aes(xintercept=trait_means_SD$mean_friendly[trait_means_SD$Country == "EA"]), colour = "red", size = 1.5) +
  geom_vline(data=filter(Data_noNeut, Culture=="Taiwanese"), aes(xintercept=trait_means_SD$mean_friendly[trait_means_SD$Country == "KR"]), colour = "light grey", size = 1.5) +
  geom_vline(data=filter(Data_noNeut, Culture=="Taiwanese"), aes(xintercept=trait_means_SD$mean_friendly[trait_means_SD$Country == "CN"]), colour = "grey", size = 1.5) +
  geom_vline(data=filter(Data_noNeut, Culture=="Taiwanese"), aes(xintercept=trait_means_SD$mean_friendly[trait_means_SD$Country == "TW"]), colour = "dark blue", size = 1.5)
...
ggplot(Data_noNeut, aes(x = trait_competent, fill = Culture)) +
  ...
  xlab("Competent") +
  coord_cartesian(x = c(1,7)) +
  geom_vline(data=filter(Data_noNeut, Culture=="European American"), aes(xintercept=trait_means_SD$mean_competent[trait_means_SD$Country == "EA"]), colour = "red", size = 1.5) +
  geom_vline(data=filter(Data_noNeut, Culture=="Taiwanese"), aes(xintercept=trait_means_SD$mean_competent[trait_means_SD$Country == "KR"]), colour = "light grey", size = 1.5) +
  geom_vline(data=filter(Data_noNeut, Culture=="Taiwanese"), aes(xintercept=trait_means_SD$mean_competent[trait_means_SD$Country == "CN"]), colour = "grey", size = 1.5) +
  geom_vline(data=filter(Data_noNeut, Culture=="Taiwanese"), aes(xintercept=trait_means_SD$mean_competent[trait_means_SD$Country == "TW"]), colour = "dark blue", size = 1.5)
```
...
competent <- lmer(data = Data, trait_competent_i ~ 1 + Culture * reputation * expression + Gender + Age + FamSES2num + (1|pID), REML=FALSE)
summary(competent)

trustworthy <- lmer(data = Data, trait_trustworthy_i ~ 1 + Culture * reputation * expression + Gender + Age + FamSES2num + (1|pID), REML=FALSE)
summary(trustworthy)

financiallyNeedy <- lmer(data = Data, trait_financiallyNeedy_i ~ 1 + Culture * reputation * expression + Gender + Age + FamSES2num + (1|pID), REML=FALSE)
summary(financiallyNeedy)

dominant <- lmer(data = Data, trait_dominant_i ~ 1 + Culture * reputation * expression + Gender + Age + FamSES2num + (1|pID), REML=FALSE)
summary(dominant)

friendly <- lmer(data = Data, trait_friendly_i ~ 1 + Culture * reputation * expression + Gender + Age + FamSES2num + (1|pID), REML=FALSE)
summary(friendly)

# Run ANOVA version: friendliness

aov_ipsatized <- aov(data = Data_noNeut, trait_friendly_i ~ Culture * reputation * expression + Error(pID/(reputation*expression)), na.action = na.exclude) # dv = response_i
summary(aov_ipsatized) # get main effects
eta_sq(aov_ipsatized, partial = TRUE) # get eta squared for partial effects
...
# Run ANOVA version: trustworthiness

aov_ipsatized <- aov(data = Data_noNeut, trait_trustworthy_i ~ Culture * reputation * expression + Error(pID/(reputation*expression)), na.action = na.exclude) # dv = response_i
summary(aov_ipsatized) # get main effects
eta_sq(aov_ipsatized, partial = TRUE) # get eta squared for partial effects
...
traitRatingsLong <- Data %>% 
  pivot_longer(cols = starts_with("trait_") & ends_with("i"), names_to = "Trait", values_to = "Value")

# ggplot(traitRatingsLong, aes(x = Trait, y = Value, fill = Culture)) +
#   geom_bar(position="dodge", stat = "summary", fun = "mean") +
#   stat_summary(geom = "errorbar", fun.data = mean_se, position=position_dodge(.9), width = 0.2) +
#   guides(fill=guide_legend(title="Culture")) +
#   scale_x_discrete(limits=c("trait_competent_i","trait_trustworthy_i", "trait_financiallyNeedy_i","trait_dominant_i", "trait_friendly_i"),
...
ggplot(Data, aes(x = exact_value, y = trait_competent_i, color = Culture)) +
  geom_jitter(alpha = .2, height = .5, width = 5) +
  geom_smooth(method = "loess") +
  scale_color_manual(values=c("#94B3D7", "#D99594")) +
  xlab("Reputation")

ggplot(Data, aes(x = exact_value, y = trait_trustworthy_i, color = Culture)) +
  geom_jitter(alpha = .2, height = .5, width = 5) +
  geom_smooth(method = "loess") +
  scale_color_manual(values=c("#94B3D7", "#D99594")) +
  xlab("Reputation")

ggplot(Data, aes(x = exact_value, y = trait_financiallyNeedy_i, color = Culture)) +
  geom_jitter(alpha = .2, height = .5, width = 5) +
  geom_smooth(method = "loess") +
  scale_color_manual(values=c("#94B3D7", "#D99594")) +
  xlab("Reputation")

ggplot(Data, aes(x = exact_value, y = trait_dominant_i, color = Culture)) +
  geom_jitter(alpha = .2, height = .5, width = 5) +
  geom_smooth(method = "loess") +
  scale_color_manual(values=c("#94B3D7", "#D99594")) +
  xlab("Reputation")

ggplot(Data, aes(x = exact_value, y = trait_friendly_i, color = Culture)) +
  geom_jitter(alpha = .2, height = .5, width = 5) +
  geom_smooth(method = "loess") +
  scale_color_manual(values=c("#94B3D7", "#D99594")) +
  xlab("Reputation")
...
ggplot(Data, aes(x = expression, y = trait_competent_i, fill = Culture)) +
  geom_bar(position="dodge", stat = "summary", fun = "mean") +
  stat_summary(geom = "errorbar", fun.data = mean_se, position=position_dodge(.9), width = 0.2) +
  coord_cartesian(ylim = c(1, 7)) +
  scale_fill_manual(values=c("#94B3D7", "#D99594")) 

ggplot(Data, aes(x = expression, y = trait_trustworthy_i, fill = Culture)) +
  geom_bar(position="dodge", stat = "summary", fun = "mean") +
  stat_summary(geom = "errorbar", fun.data = mean_se, position=position_dodge(.9), width = 0.2) +
  coord_cartesian(ylim = c(1, 7)) +
  scale_fill_manual(values=c("#94B3D7", "#D99594")) 

ggplot(Data, aes(x = expression, y = trait_financiallyNeedy_i, fill = Culture)) +
  geom_bar(position="dodge", stat = "summary", fun = "mean") +
  stat_summary(geom = "errorbar", fun.data = mean_se, position=position_dodge(.9), width = 0.2) +
  coord_cartesian(ylim = c(1, 7)) +
  scale_fill_manual(values=c("#94B3D7", "#D99594"))

ggplot(Data, aes(x = expression, y = trait_dominant_i, fill = Culture)) +
  geom_bar(position="dodge", stat = "summary", fun = "mean") +
  stat_summary(geom = "errorbar", fun.data = mean_se, position=position_dodge(.9), width = 0.2) +
  coord_cartesian(ylim = c(1, 7)) +
  scale_fill_manual(values=c("#94B3D7", "#D99594"))

ggplot(Data, aes(x = expression, y = trait_friendly_i, fill = Culture)) +
  geom_bar(position="dodge", stat = "summary", fun = "mean") +
  stat_summary(geom = "errorbar", fun.data = mean_se, position=position_dodge(.9), width = 0.2) +
  coord_cartesian(ylim = c(1, 7)) +
  scale_fill_manual(values=c("#94B3D7", "#D99594"))
...
# ggplot(Data, aes(x = exact_value, y = trait_competent_i, color = expression)) +
#   geom_jitter(alpha = .1, height = .5, width = 5) +
#   geom_smooth(method = "loess") +
...
# ggplot(Data, aes(x = exact_value, y = trait_financiallyNeedy_i, color = expression)) +
#   geom_jitter(alpha = .1, height = .5, width = 5) +
#   geom_smooth(method = "loess") +
#   scale_color_manual(values = expr_colors) +
#   xlab("Reputation")
...
summary <- Data %>%
  dplyr::select(pID, Culture, trait_trustworthy_i, expression, reputation) %>% 
  dplyr::ungroup() %>%
  dplyr::group_by(Culture, pID, expression, reputation) %>%
  dplyr::summarise(n = n(),
                   mean = mean(trait_trustworthy_i, na.rm = T),
                   stdv = sd(trait_trustworthy_i, na.rm = T),
                   sem = stdv / sqrt(n)) %>%
  # calculate across subjects
  dplyr::group_by(Culture, expression, reputation) %>%
  dplyr::summarise(n = n(),
...
FactorAnalysisWhole <- Data %>% 
  dplyr::select(contains("trait_") & ends_with("i")) %>%
  drop_na()
...
FactorAnalysisTaiwan <- Data %>% 
  dplyr::filter(Culture == "Taiwanese") %>% 
  dplyr::select(contains("trait_") & ends_with("i")) %>%
  drop_na()
...
# Taiwanese: linear trends for reputation
...
# Taiwanese: effect coding for reputation
...
competent <- lmer(data = Data_noNeut, trait_competent_i ~ 1 + Culture * reputation * expression + Gender + Age + FamSES2num + (1|pID), REML=FALSE)
summary(competent)

trustworthy <- lmer(data = Data_noNeut, trait_trustworthy_i ~ 1 + Culture * reputation * expression + Gender + Age + FamSES2num + (1|pID), REML=FALSE)
summary(trustworthy)

financiallyNeedy <- lmer(data = Data_noNeut, trait_financiallyNeedy_i ~ 1 + Culture * reputation * expression + Gender + Age + FamSES2num + (1|pID), REML=FALSE)
summary(financiallyNeedy)

dominant <- lmer(data = Data_noNeut, trait_dominant_i ~ 1 + Culture * reputation * expression + Gender + Age + FamSES2num + (1|pID), REML=FALSE)
summary(dominant)

friendly <- lmer(data = Data_noNeut, trait_friendly_i ~ 1 + Culture * reputation * expression + Gender + Age + FamSES2num + (1|pID), REML=FALSE)
summary(friendly)
```
## Visualize trait ratings 
...
ggplot(Data_noNeut, aes(x = exact_value, y = trait_competent_i, color = Culture)) +
  geom_jitter(alpha = .2, height = .5, width = 5) +
  geom_smooth(method = "loess") +
  scale_color_manual(values=c("#94B3D7", "#D99594")) +
  xlab("Reputation")

ggplot(Data_noNeut, aes(x = exact_value, y = trait_trustworthy_i, color = Culture)) +
  geom_jitter(alpha = .2, height = .5, width = 5) +
  geom_smooth(method = "loess") +
  scale_color_manual(values=c("#94B3D7", "#D99594")) +
  xlab("Reputation")

ggplot(Data_noNeut, aes(x = exact_value, y = trait_financiallyNeedy_i, color = Culture)) +
  geom_jitter(alpha = .2, height = .5, width = 5) +
  geom_smooth(method = "loess") +
  scale_color_manual(values=c("#94B3D7", "#D99594")) +
  xlab("Reputation")

ggplot(Data_noNeut, aes(x = exact_value, y = trait_dominant_i, color = Culture)) +
  geom_jitter(alpha = .2, height = .5, width = 5) +
  geom_smooth(method = "loess") +
  scale_color_manual(values=c("#94B3D7", "#D99594")) +
  xlab("Reputation")

# ggplot(Data_noNeut, aes(x = reputation, y = trait_dominant_i, fill = Culture)) +
#   geom_bar(position="dodge", stat = "summary", fun.y = "mean") +
#   stat_summary(geom = "errorbar", fun.data = mean_se, position=position_dodge(.9), width = 0.2) +
#   coord_cartesian(ylim = c(1, 7)) +
#   scale_fill_manual(values=c("#94B3D7", "#D99594"))

ggplot(Data_noNeut, aes(x = exact_value, y = trait_friendly_i, color = Culture)) +
  geom_jitter(alpha = .2, height = .5, width = 5) +
  geom_smooth(method = "loess") +
  scale_color_manual(values=c("#94B3D7", "#D99594")) +
  xlab("Reputation")
...
ggplot(Data_noNeut, aes(x = expression, y = trait_competent_i, fill = Culture)) +
  geom_bar(position="dodge", stat = "summary", fun.y = "mean") +
  stat_summary(geom = "errorbar", fun.data = mean_se, position=position_dodge(.9), width = 0.2) +
  coord_cartesian(ylim = c(1, 7)) +
  scale_fill_manual(values=c("#94B3D7", "#D99594")) 

ggplot(Data_noNeut, aes(x = expression, y = trait_trustworthy_i, fill = Culture)) +
  geom_bar(position="dodge", stat = "summary", fun.y = "mean") +
  stat_summary(geom = "errorbar", fun.data = mean_se, position=position_dodge(.9), width = 0.2) +
  coord_cartesian(ylim = c(1, 7)) +
  scale_fill_manual(values=c("#94B3D7", "#D99594")) 

ggplot(Data_noNeut, aes(x = expression, y = trait_financiallyNeedy_i, fill = Culture)) +
  geom_bar(position="dodge", stat = "summary", fun.y = "mean") +
  stat_summary(geom = "errorbar", fun.data = mean_se, position=position_dodge(.9), width = 0.2) +
  coord_cartesian(ylim = c(1, 7)) +
  scale_fill_manual(values=c("#94B3D7", "#D99594"))

ggplot(Data_noNeut, aes(x = expression, y = trait_dominant_i, fill = Culture)) +
  geom_bar(position="dodge", stat = "summary", fun.y = "mean") +
  stat_summary(geom = "errorbar", fun.data = mean_se, position=position_dodge(.9), width = 0.2) +
  coord_cartesian(ylim = c(1, 7)) +
  scale_fill_manual(values=c("#94B3D7", "#D99594"))

ggplot(Data_noNeut, aes(x = expression, y = trait_friendly_i, fill = Culture)) +
  geom_bar(position="dodge", stat = "summary", fun.y = "mean") +
  stat_summary(geom = "errorbar", fun.data = mean_se, position=position_dodge(.9), width = 0.2) +
  coord_cartesian(ylim = c(1, 7)) +
  scale_fill_manual(values=c("#94B3D7", "#D99594"))

# ggplot(Data_noNeut, aes(x = reputation, y = trait_friendly_i, fill = expression)) +
#   geom_bar(position="dodge", stat = "summary", fun.y = "mean") +
#   stat_summary(geom = "errorbar", fun.data = mean_se, position=position_dodge(.9), width = 0.2) +
#   coord_cartesian(ylim = c(1, 7)) +
#   scale_fill_manual(values=c("#94B3D7", "#D99594"))
...
# ggplot(Data_noNeut, aes(x = exact_value, y = trait_competent_i, color = expression)) +
#   geom_jitter(alpha = .1, height = .5, width = 5) +
#   geom_smooth(method = "loess") +
#   scale_color_manual(values = expr_colors) +
#   xlab("Reputation")
...
# ggplot(Data_noNeut, aes(x = exact_value, y = trait_financiallyNeedy_i, color = expression)) +
#   geom_jitter(alpha = .1, height = .5, width = 5) +
#   geom_smooth(method = "loess") +
#   scale_color_manual(values = expr_colors) +
#   xlab("Reputation")
...
# ggplot(Data_noNeut, aes(x = exact_value, y = trait_friendly_i, color = expression)) +
#   geom_jitter(alpha = .1, height = .5, width = 5) +
#   geom_smooth(method = "loess") +
#   scale_color_manual(values = expr_colors) +
#   xlab("Reputation")
...
(2) subtracting participants’ transfers to calm recipients from those to excited recipients, collapsing across recipient reputation, race and sex
...
computeDiffs <- Data_noNeut %>% 
  dplyr::select(contains("trait_") & ends_with("i"), pID, expression, response_i) %>% 
  group_by(pID, expression) %>% 
  dplyr::summarise(response_i_mean = mean(response_i, na.rm = T),
            competence_i = mean(trait_competent_i, na.rm = T),
            trustworthy_i = mean(trait_trustworthy_i, na.rm = T),
            financiallyNeedy_i = mean(trait_financiallyNeedy_i, na.rm = T),
            dominant_i = mean(trait_dominant_i, na.rm = T),
            friendly_i = mean(trait_friendly_i, na.rm = T)) %>% 
  pivot_wider(names_from = expression, values_from = c(response_i_mean, competence_i, trustworthy_i, financiallyNeedy_i, dominant_i, friendly_i)) %>% 
  dplyr::mutate(responseRatio = response_i_mean_exci/response_i_mean_calm,
         responseDiff_i = response_i_mean_exci - response_i_mean_calm,
         competenceDiff_i = competence_i_exci - competence_i_calm,
         trustworthyDiff_i = trustworthy_i_exci - trustworthy_i_calm,
         financiallyNeedyDiff_i = financiallyNeedy_i_exci - financiallyNeedy_i_calm,
         dominantDiff_i = dominant_i_exci - dominant_i_calm,
         friendlyDiff_i = friendly_i_exci - friendly_i_calm)
...
ggplot(mediationData, aes(x = Culture, y = responseDiff_i)) +
  geom_point()
```
...
```{r}
# step 1: x -> y 
summary(lm(responseDiff_i ~ Culture, data = mediationData))

# step 2: x -> m 
summary(lm(competenceDiff_i ~ Culture, data = mediationData))

# step 3: x -> m 
summary(lm(responseDiff_i ~ competenceDiff_i + Culture, data = mediationData))

# set up models
model.M <- lm(competenceDiff_i ~ Culture, data = mediationData)
model.Y <- lm(responseDiff_i ~ Culture + competenceDiff_i, data = mediationData)

results <- mediation::mediate(model.M, model.Y, treat='Culture', mediator='competenceDiff_i', boot = TRUE, sims=1000, control.value="Taiwanese", treat.value="European American")
summary(results)

# for reporting mediator --> DV arrow
summary(lm(responseDiff_i ~ competenceDiff_i, data = mediationData))
```
...
# step 2: x -> m 
summary(lm(trustworthyDiff_i ~ Culture, data = mediationData))

# step 3: x -> m 
summary(lm(responseDiff_i ~ trustworthyDiff_i + Culture, data = mediationData))

# set up models
model.M <- lm(trustworthyDiff_i ~ Culture, data = mediationData)
model.Y <- lm(responseDiff_i ~ Culture + trustworthyDiff_i, data = mediationData)

results <- mediation::mediate(model.M, model.Y, treat='Culture', mediator='trustworthyDiff_i', boot = TRUE, sims=1000, control.value="Taiwanese", treat.value="European American")
summary(results)

# for reporting mediator --> DV arrow
summary(lm(responseDiff_i ~ trustworthyDiff_i, data = mediationData))
```
...
# step 2: x -> m 
summary(lm(financiallyNeedyDiff_i ~ Culture, data = mediationData))

# step 3: x -> m 
summary(lm(responseDiff_i ~ financiallyNeedyDiff_i + Culture, data = mediationData))

# set up models
model.M <- lm(financiallyNeedyDiff_i ~ Culture, data = mediationData)
model.Y <- lm(responseDiff_i ~ Culture + financiallyNeedyDiff_i, data = mediationData)

results <- mediation::mediate(model.M, model.Y, treat='Culture', mediator='financiallyNeedyDiff_i', boot = TRUE, sims=1000, control.value="Taiwanese", treat.value="European American")
summary(results)

# for reporting mediator --> DV arrow
summary(lm(responseDiff_i ~ financiallyNeedyDiff_i, data = mediationData))
```
...
# step 2: x -> m 
summary(lm(dominantDiff_i ~ Culture, data = mediationData))

# step 3: x -> m 
summary(lm(responseDiff_i ~ dominantDiff_i + Culture, data = mediationData))

# set up models
model.M <- lm(dominantDiff_i ~ Culture, data = mediationData)
model.Y <- lm(responseDiff_i ~ Culture + dominantDiff_i, data = mediationData)

results <- mediation::mediate(model.M, model.Y, treat='Culture', mediator='dominantDiff_i', boot = TRUE, sims=1000, control.value="Taiwanese", treat.value="European American")
summary(results)

# for reporting mediator --> DV arrow
summary(lm(responseDiff_i ~ dominantDiff_i, data = mediationData))
```
...
# step 2: x -> m 
summary(lm(friendlyDiff_i ~ Culture, data = mediationData))

# step 3: x -> m 
summary(lm(responseDiff_i ~ friendlyDiff_i + Culture, data = mediationData))

# set up models
model.M <- lm(friendlyDiff_i ~ Culture, data = mediationData)
model.Y <- lm(responseDiff_i ~ Culture + friendlyDiff_i, data = mediationData)

results <- mediation::mediate(model.M, model.Y, treat='Culture', mediator='friendlyDiff_i', boot = TRUE, sims=1000, control.value="Taiwanese", treat.value="European American")
summary(results)

# for reporting mediator --> DV arrow
summary(lm(responseDiff_i ~ friendlyDiff_i, data = mediationData))
```
...
computeDiffs <- Data_noNeut %>% 
  dplyr::select(contains("trait_") & ends_with("i"), pID, expression, reputation, response_i) %>% 
  group_by(pID, expression, reputation) %>% 
  dplyr::summarise(response_i_mean = mean(response_i, na.rm = T),
            competence_i = mean(trait_competent_i, na.rm = T),
            trustworthy_i = mean(trait_trustworthy_i, na.rm = T),
            financiallyNeedy_i = mean(trait_financiallyNeedy_i, na.rm = T),
...
The processr package (https://rpubs.com/markhw/processr) uses lavaan to run mediations (here I will run the moderated mediation, hayes' process model 7 using the function model7). 
```{r}
# remotes::install_github("markhwhiteii/processr")
library(processr)

# prepare the data

# Make factors numeric
mediationData <- processr::make_numeric(data=mediationData, var = "Culture", ref = "Taiwanese", newvar = "Culture_num")
# Make reputation numeric and only two levels (1 = high, -1 = mod/low)
mediationData <- mediationData %>%  
  dplyr::mutate(reputation = ifelse(reputation == "high", 1, 0))

# mean center traits
mediationData$competenceDiff_i.c <- scale(mediationData$competenceDiff_i, center = TRUE, scale = FALSE)[,] #Scale returns a matrix so we have to make it a vector by indexing one column
mediationData$trustworthyDiff_i.c <- scale(mediationData$trustworthyDiff_i, center = TRUE, scale = FALSE)[,] 
mediationData$financiallyNeedyDiff_i.c <- scale(mediationData$financiallyNeedyDiff_i, center = TRUE, scale = FALSE)[,]
mediationData$dominantDiff_i.c <- scale(mediationData$dominantDiff_i, center = TRUE, scale = FALSE)[,] 
mediationData$friendlyDiff_i.c <- scale(mediationData$friendlyDiff_i, center = TRUE, scale = FALSE)[,] 

trait_competence <- model7(iv = "Culture_num", dv ="responseDiff_i", med = "competenceDiff_i.c", mod = "reputation", data = mediationData, samples = 5000)
kable(trait_competence)

trait_trustworthy <- model7(iv = "Culture_num", dv ="responseDiff_i", med = "trustworthyDiff_i.c", mod = "reputation", data = mediationData, samples = 5000)
kable(trait_trustworthy)

trait_financiallyNeedy <- model7(iv = "Culture_num", dv ="responseDiff_i", med = "financiallyNeedyDiff_i.c", mod = "reputation", data = mediationData, samples = 5000)
kable(trait_financiallyNeedy)

trait_dominant <- model7(iv = "Culture_num", dv ="responseDiff_i", med = "dominantDiff_i.c", mod = "reputation", data = mediationData, samples = 5000)
kable(trait_dominant)

trait_friendly <- model7(iv = "Culture_num", dv ="responseDiff_i", med = "friendlyDiff_i.c", mod = "reputation", data = mediationData, samples = 5000)
kable(trait_friendly)

# # unintuitive models:

# trait_friendly_model14 <- model14(iv = "Culture_num", dv ="responseDiff_i", med = "friendlyDiff_i.c", mod = "reputation", data = mediationData, samples = 5000)
# 
# kable(trait_friendly_model14)

# trait_trustworthy_model14 <- model14(iv = "Culture_num", dv ="responseDiff_i", med = "trustworthyDiff_i.c", mod = "reputation", data = mediationData, samples = 5000)
# 
# kable(trait_trustworthy_model14)
```

```{r}
# plot
ggplot(mediationData, aes(x = reputation, y = friendlyDiff_i, fill = Culture)) +
  geom_bar(position="dodge", stat = "summary", fun.y = "mean") +
  stat_summary(geom = "errorbar", fun.data = mean_se, position=position_dodge(.9), width = 0.2) 

ggplot(mediationData, aes(x = reputation, y = trustworthyDiff_i, fill = Culture)) +
  geom_bar(position="dodge", stat = "summary", fun.y = "mean") +
  stat_summary(geom = "errorbar", fun.data = mean_se, position=position_dodge(.9), width = 0.2) 

ggplot(mediationData, aes(x = reputation, y = responseDiff_i, fill = Culture)) +
  geom_bar(position="dodge", stat = "summary", fun.y = "mean") +
  stat_summary(geom = "errorbar", fun.data = mean_se, position=position_dodge(.9), width = 0.2) 
```
